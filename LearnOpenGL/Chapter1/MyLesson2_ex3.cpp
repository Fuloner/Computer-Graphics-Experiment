...
int main()
{
    ...
    //创建和编译片段着色器1
    unsigned int fragmentShader1; //片段着色器对象ID
    fragmentShader1 = glCreateShader(GL_FRAGMENT_SHADER); //创建一个片段着色器对象 参数为着色器类型 创建片段着色器的参数是GL_FRAGMENT_SHADER
    glShaderSource(fragmentShader1, 1, &fragmentShaderSource1, NULL); //将片段着色器源码附加到着色器对象上
    glCompileShader(fragmentShader1); //编译着色器对象
    //检查编译是否成功
    ...
    //创建和编译片段着色器2
    unsigned int fragmentShader2; //片段着色器对象ID
    fragmentShader2 = glCreateShader(GL_FRAGMENT_SHADER); //创建一个片段着色器对象 参数为着色器类型 创建片段着色器的参数是GL_FRAGMENT_SHADER
    glShaderSource(fragmentShader2, 1, &fragmentShaderSource2, NULL); //将片段着色器源码附加到着色器对象上
    ...
    //链接着色器程序
    unsigned int shaderProgram1, shaderProgram2; //着色器程序对象ID
    ...
    while(!glfwWindowShouldClose(window)) //主循环 直到用户关闭窗口
    {
        ...
        glUseProgram(shaderProgram1); //激活着色器程序1 参数为程序对象ID 之后所有的渲染调用都会使用这个程序对象
        glBindVertexArray(VAO[0]); //绑定VAO 参数为要绑定的VAO的ID
        glDrawArrays(GL_TRIANGLES, 0, 3); //绘制图元 参数为图元类型、起始索引和顶点数量
        glUseProgram(shaderProgram2); //激活着色器程序2 参数为程序对象ID 之后所有的渲染调用都会使用这个程序对象
