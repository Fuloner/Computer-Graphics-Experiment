...
int main()
{
    ...
    //--设置顶点属性指针--
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0); //设置顶点属性指针 参数为属性位置、属性大小、数据类型、是否归一化、步长和偏移量
    ...
    while(!glfwWindowShouldClose(window)) //主循环 直到用户关闭窗口
    {
        ...
        int vertexColorLocation = glGetUniformLocation(shaderProgram, "ourColor"); //获取uniform变量的位置 参数为程序对象ID 和 变量名
        glUniform4f(vertexColorLocation, 0.0f, greenValue, 0.0f, 1.0f); //设置uniform变量的值 参数为变量位置 和 变量值
                                                                        //更新uniform值之前 必须先激活着色器程序glUseProgram() 因为它是在当前激活的着色器程序中设置uniform的
